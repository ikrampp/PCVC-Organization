	...
	icons: {
		jQueryUI: {
			...
			nav: {
				...
				cancel: "glyphicon glyphicon-floppy-remove",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group' style='height:60px;'><table "+(isMSIE7 ? "cellspacing='0' " : "")+"class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;'><tbody><tr><td id='"+lft+"' style='text-align:center;'></td></tr></tbody></table></div>")
			...
			if(dir==="ltr") { pgl += str; }*/
			pgl += "</tr></tbody></table>";
			if(p.viewrecords===true) {$("td"+pgid+"_"+p.recordpos,pgcnt).append("<div dir='"+dir+"' style='text-align:center"+p.recordpos+"' class='ui-paging-info'></div>");}
...
//Helper functions
	...
	createModal : function(aIDs, content, o, insertSelector, posSelector, appendsel, css) {
		...
		.hover(function(){ahr.addClass('ui-but-hover');},
...
jgrid.extend({
	...
	editGridRow : function(rowid, oMuligrid){		// if one uses jQuery wrapper with multiple grids, then oMultiple specify the object with common options
		return this.each(function(){
			...
			bS  ="<a id='sData' class='btn btn-md btn-primary ui-corner-all'><span class='fm-button-text'>"+o.bSubmit+"</span></a>",
			bC  ="<a id='cData' class='btn btn-md btn-default ui-corner-all'><span class='fm-button-text'>"+o.bCancel+"</span></a>";
		...
		return this.each(function() {
			...
			stdButtonActivation = function (name, id, onClick, navtbl, elemids) {
				...
				$button.append("<div class='ui-pg-div'><span class='" +
					(o.iconsOverText ?
						mergeCssClasses("ui-pg-button-icon-over-text") :
						"") +
					"'><span class='btn btn-xm btn-primary'>Add new row</span></span>" +
			...
			for(i = 0; i<clone; i++) {
				navtbl = $("<div"+" class='ui-pg-table navtable' style='float:center;table-layout:auto;'></div>");
				if(i===0) {
					pgid = elem;
					elemids = gridId;
					if(pgid === p.toppager) {
	...
	inlineNav : function (elem, oMuligrid) {
		...
		return this.each(function(){
			...
			o = $.extend(true,{
				...
				cancel: true,
				cancelicon:"glyphicon glyphicon-floppy-remove",
				commonIconClass : "",
				iconsOverText : false,
...
addSubGridCell :function (pos,iRow) {
	...
	return self == null || self.p == null || self.p.subGridOptions == null ? "" :
		"<td style=padding:5px role=\"gridcell\" aria-describedby=\""+self.p.id+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+
		...
		var onClick = function() {
			...
			if($(this).hasClass("sgcollapsed")) {
				if(p.subGridOptions.reloadOnExpand === true || ( p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
					atd = pos >=1 ? "<td class='ui-widget-content' colspan='"+pos+"'>&#160;</td>":"";
...
(function($) {
	...
	$FnFmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=\"return jQuery.fn.fmatter.rowactions.call(this,event,'formedit');\" onmouseover=jQuery(this).addClass('ui-but-hover'); onmouseout=jQuery(this).removeClass('ui-but-hover'); ";
			str += "<div title='"+op.edittitle+"' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='" + cssIconClass("edit") + "'></span></div>";
		} else if(op.editbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=\"return jQuery.fn.fmatter.rowactions.call(this,event,'edit');\" onmouseover=jQuery(this).addClass('ui-but-hover'); onmouseout=jQuery(this).removeClass('ui-but-hover') ";
			str += "<div title='"+op.edittitle+"' class='btn btn-md btn-primary ui-inline-edit' "+ocl+"><span class='" + cssIconClass("edit") + "'></span></div>";
		}
		if(op.delbutton) {
			ocl = "id='jDeleteButton_"+rowid+"' onclick=\"return jQuery.fn.fmatter.rowactions.call(this,event,'del');\" onmouseover=jQuery(this).addClass('ui-but-hover'); onmouseout=jQuery(this).removeClass('ui-but-hover'); ";
			str += "<div title='"+op.deltitle+"' class='ui-pg-div ui-inline-del' "+ocl+"><span class='" + cssIconClass("del") + "'></span></div>";
		}
		ocl = "id='jSaveButton_"+rowid+"' onclick=\"return jQuery.fn.fmatter.rowactions.call(this,event,'save');\" onmouseover=jQuery(this).addClass('ui-but-hover'); onmouseout=jQuery(this).removeClass('ui-but-hover'); ";
		str += "<div title='"+op.savetitle+"' style='display:none' class='ui-pg-div ui-inline-save' "+ocl+"><span class='" + cssIconClass("save") + "'></span></div>";
		ocl = "id='jCancelButton_"+rowid+"' onclick=\"return jQuery.fn.fmatter.rowactions.call(this,event,'cancel');\" onmouseover=jQuery(this).addClass('ui-but-hover'); onmouseout=jQuery(this).removeClass('ui-but-hover'); ";
